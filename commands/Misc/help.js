...
async function help(message, args) {
    ...
    if (args.length === 0) {
        ...
        fs.readdirSync('./commands/').forEach((dir) => { // for each folders inside commands folder
            ...
            const cmds = commands.map((command) => { // map commands file
                ...
                return { // return object with command name and description property
                    ...
                    description: banmoi.commands.get(name).description // get the command description by command name, from the commands collection
                }
            });
        ...
        const command = banmoi.commands.get(args.join(' ').toLowerCase()) || banmoi.commands.find(cmd => cmd.aliases && cmd.aliases.includes(args.join(' ').toLowerCase())); // find if commands collection or aliases inside command data includes the arguments of your message
